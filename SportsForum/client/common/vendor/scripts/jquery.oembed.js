 ...
 */
(function($) {
    $.fn.oembed = function(url, options, embedAction, parent, index) {
        settings = $.extend(true, $.fn.oembed.defaults, options);
        theUrl = url;
        parent.append('<div class="oembed" id="'+index+'">')
					...
					var ajaxopts = $.extend({
						  ...
						  success: function(data) {
							...
							if (provider !== null) {
								...
								embedCode(container, resourceURL, provider, parent, index);
							} else {
    ...
    function embedCode(container, externalUrl, embedProvider, parent, index) {
      if ($('#jqoembeddata').data(externalUrl)!=undefined && embedProvider.embedtag.tag!='iframe'){
        var oembedData = {code: $('#jqoembeddata').data(externalUrl)};
        oembedData.index = index
        oembedData.parent = parent
        oembedData.externalUrl = externalUrl
    ...
    $.fn.oembed.insertCode = function(container, embedMethod, oembedData) {
        ...
        switch (embedMethod) {
          ...
          case "fill":
              ...
              break;
          case "append":
              //$text = container.parents('.text')
              //$text = oembedData.parent
              //oembedContainer = $('<div class="oembed"></div>')
              oembedContainer = oembedData.parent.find('#'+oembedData.index);
    ...
    $.fn.oembed.getPhotoCode = function(url, oembedData) {
        ...
        if (oembedData.url){
            if(result = oembedData.url.match(/(mp4|webm|ogg)$/ig)){//A video
              var type = 'video/'+result[0]
              code = '<div><video class="embedVideo" controls><source src="'+oembedData.url+'" type="'+type+'">Your browser cannot embed this video.</video></div>';
              }
            else{
            code = '<div><a href="' + url + '" target=\'_blank\'><img class="embedImage" src="' + oembedData.url + '" alt="' + alt + '"/></a></div>';
            }
    ...
    {yql:{xpath:"//meta|//title|//link", from:'html'
        , datareturn:function(results){
            ...
            if(!results['og:description'] && results['test quotes']) results['og:description']=results['test quotes'];
            if(!results['og:description'] && results['description'] && results['title'])results['og:description']=results['description'];
            if(!results['og:title'] && !results['title'])return false;
            var code = $('<tr></tr>');
            var firstCellCode = undefined
            var secondCellCode = $('<td class="openGraphCell" style="vertical-align:top"></td>');
            if(results['og:video']) {
              ...
              var img = $('<img class="embedThumbnail" src="'+results['og:image']+'">');
              firstCellCode = img;
            }
            if(firstCellCode !== undefined){
              code.append($('<td class="openGraphCell"></td>').append(firstCellCode))
            }

            if(results['og:title']) secondCellCode.append('<div class="embedTitle">'+results['og:title']+'</span>');
            if(results['og:site_name']) secondCellCode.append('<div class="embedSiteName">'+results['og:site_name']+'</div>');
            if(results['og:description']) secondCellCode.append('<div class="embedDescription">'+results['og:description']+'</div>');
            code.append(secondCellCode)
            code = $('<table class="openGraphEmbed"></table>').append(code);
	          if(!results['og:url']) results['og:url'] = theUrl;

            if (firstCellCode !== undefined) firstCellCode.wrap('<a href="'+results['og:url']+'" target=\'_blank\'></a>');
            secondCellCode.find('.embedTitle').wrap('<a href="'+results['og:url']+'" target=\'_blank\'></a>');
            return code.html();
          }
        }
    })
