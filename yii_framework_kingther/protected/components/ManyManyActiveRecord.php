...
class ManyManyActiveRecord extends CActiveRecord {
     ...
     */
    public function setRelationRecords($relationName, $relationData, $additionalFieldsArray = array(), $useTransaction = false) {
        ...
        try {
            ...
            $sql = array();
            for ($i = 0; $i < $c; $i++) {
                if (count($additionalFieldsArray) > 0 && isset($additionalFieldsArray[$i])) {
                    $keys = array();
                    foreach ($additionalFieldsArray[$i] as $key => $value) {
                        $keys[] = $key;
                    }
     ...
     * @param array	    $relationData array of related keys of second table to be connected with first table
     */
    public function addRelationRecords($relationName, $relationData, $additionalFieldsArray = array()) {
        //get correct relation from model relation defenition
        $relation = $this->getActiveRelation($relationName);
