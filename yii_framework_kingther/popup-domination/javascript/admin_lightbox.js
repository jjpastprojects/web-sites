;(function($){
	...
	var list_count = 0, tmpimg, to_show = '', still_over = false, last_checked = [], tempchange = false, firstload = true;
	$(document).ready(function(){
		...
		$('#popup_domination_promote').change(function(){
			$('#popup_domination_clickbank').attr('disabled',!($(this).is(':checked')));
		});
		...
		$('.cookieclear').click(function(){
			...
			waiting.show();
			$.get('index.php?section=ajax&action=ajax&do=clear_cookie&id='+popup_domination_campaign_id,function(resp){
				waiting.hide();
			},'json');
			return false;
		});
		
		set_template(firstload);
		...
		$('#popdom_contentbox_inside .button').click(function(){
			...
			var opts = {"action":'popup_domination_clear_cookie',
						"_wpnonce": $('#_wpnonce').val(),
						"_wp_http_referer": $('input[name="_wp_http_referer"]').val()};
			$.get('index.php?section=ajax&action=ajax&do=clear_cookie'+'?rand='+(Math.random()*555),function(resp){
				waiting.hide();
			},'json');
			return false;
		});
	...
	function checkcampname(){
		...
		jQuery.post('index.php?section=ajax&action=ajax&do=checkabname', data, function(response) {
			$('#campname').removeClass();
			$('#popup_domination_tabs .campaign-name-box .waiting').hide();
			response = $.trim(response);
			if(response != 'false'){
				if(response == popup_domination_campaign_id){
					//$('#popup_domination_tabs .campaign-name-box .confirm').show();
					//alert('This name is a-ok! :) ');
					$('#campname').addClass('name_confirm');
					$('.removeme').hide();
					
					$('.savecamp').removeAttr('disabled');
				}else{
					//$('#popup_domination_tabs .campaign-name-box .error').show();
	...
	function set_max_msg(obj){
		...
		var length = $(fieldtype,obj).val().length, elem;
	...
	function init_upload(field){
		...
		new AjaxUpload($(elem).find('a[href$="#upload_file"]'),{
			action: 'index.php?section=ajax&action=ajax&do=file_upload',
			name: 'userfile',
			data: {
	...
	function do_preview(){
		...
		elem.attr('action','index.php?section=ajax&action=ajax&do=preview')
		...
		if(theme.colors && theme.colors.length > 0){
			...
			if(tempchange){
				...
				$.each(theme.colors, function(index, value){
					...
					var selectedcol = $('.selectedcolor').val();
					if(value.options[0] == selectedcol){
						$('#popup_domination_color_selected').append('<option selected="selected" class="'+value.options+'">'+value.name+'</option>');
					}else{
				...
				$.each(theme.button_colors, function(index, value){
					$('#popup_domination_btn_color').append('<option class="'+value.color_id+'" value="'+value.color_id+'">'+value.name+'</option>');
			...
			$('#popup_domination_btn_color option').each(function(){
				if($(this).attr('class') == buttoncolor){
		...
		if(theme.fields && theme.fields.length > 0){
			...
			$.each(theme.fields,function(){
				...
				if(elem.length == 0){
					...
					} else if(this.opts.type == 'videoembed'){
						elem.append('<input type="'+this.opts.type+'" name="popup_domination_fields['+this.opts.id+']" id="popup_domination_field_'+this.opts.id+'_field" />'+str+'<br/>Looking for an AWESOME video player? Check out <a href="http://popdom.webactix.hop.clickbank.net/" target="_blank">Easy Video Player 2.0</a>.');
					} else {
					...
					//var elem2 = elem.html();
					//alert(elem.find(fieldtype).val());
					//elem.remove();
					//elem = $('<p id="popup_domination_field_'+this.opts.id+'"></p>').html(elem2);
					if(lastelem === null){
						tpltab.prepend(elem);
					} else {
						lastelem.after(elem);
