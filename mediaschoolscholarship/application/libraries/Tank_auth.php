...
class Tank_auth
{
	...
	function login($email, $password)
	{
		...
		if ((strlen($email) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_email($email))) {	// login ok without password check
                            
                                if ($user->approved==PENDING){  //check whether admin or not
                                    
                                    $this->error = array('level' => 'User is not approved.');
                                }
                                else if ($user->status==0){  //check whether admin or not
                                    
                                    $this->error = array('level' => 'User is not enabled.');
                                }
                                else{
                                    // Does password match hash in database?
                                  
                                    if ($user->password==$password) {		// password ok
                                            ...
                                            $this->error = array('level' => 'Password doesn\'t match');
                                    }
                                }
