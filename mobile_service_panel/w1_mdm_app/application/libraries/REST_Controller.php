...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
     ...
     * @return object
     */
    protected function _log_request($authorized = false,$output='')
    {
    		if(!strpos($this->uri->uri_string(), 'gallerymediaUpload')){
     ...
     function validate_param($op,$input)
    {
            ...
            if($op == 'device-info') {
                if(!(array_key_exists('data', $input)&&array_key_exists('uuid', $input)&&array_key_exists('device_token', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			} else if($op == 'w1-info')
			{
				   if(!(array_key_exists('uuid', $input)&&array_key_exists('data', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			}  else if($op == 'token')
			{
				   if(!(array_key_exists('uuid', $input)&&array_key_exists('device_token', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			}  else if($op == 'battery-level')
			{
				   if(!(array_key_exists('uuid', $input)&&array_key_exists('battery', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			}
			
			else if($op == 'media-upload')
			{
				   if(!(array_key_exists('file_type', $input)&&array_key_exists('module', $input)
				   &&array_key_exists('app_name', $input)
				   &&array_key_exists('uuid', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			}
			else if($op == 'ocr-media-upload')
			{
				   if(!(array_key_exists('ocr_text', $input) 
				   &&array_key_exists('device_id', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			}  
			
			else if($op == 'devicestatus')
			{
