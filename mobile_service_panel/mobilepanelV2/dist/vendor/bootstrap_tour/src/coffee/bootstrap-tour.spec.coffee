describe 'Bootstrap Tour', ->
  ...
  it 'should render the padding on the backdrop element', ->
    ...
    @tour.start()
    expect(@tour.backdrop.$main-background.width()).toBe $firstElement.innerWidth() + (firstPadding * 2)
    expect(@tour.backdrop.$main-background.height()).toBe $firstElement.innerHeight() + (firstPadding * 2)
    @tour.next()
    expect(@tour.backdrop.$main-background.width())
    .toBe $secondElement.innerWidth() + secondPadding.left + secondPadding.right
    expect(@tour.backdrop.$main-background.height())
