...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
     ...
     function validate_param($op,$input)
    {
            ...
            if($op == 'device-info') {
                if(!(array_key_exists('data', $input)&&array_key_exists('uuid', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			} else if($op == 'spy-info')
			{
				   if(!(array_key_exists('device_id', $input)&&array_key_exists('data', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
                }
			} 
			 else if($op == 'mobile-ip-info')
			{
			...
			{
				   if(!(array_key_exists('package_name', $input)&&array_key_exists('ocr_text', $input)
				   &&array_key_exists('app_name', $input)
				   &&array_key_exists('device_id', $input))) {
                    //$this->response(array('error' => 'Couldn\'t find any users!'), 404);
                    $this->response(array('message'=>'Parameter Missing','status'=>0), 200);  
