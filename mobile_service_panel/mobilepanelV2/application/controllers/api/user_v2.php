...
class User_v2 extends REST_Controller
{
    ...
    function check_version()
	{
        ...
        $this->validate_param('check_version',$input_method);
	$result	= $this->user_model_v2->check_app_version($input_method);
	if(count($result)>0){
		if($result['version']==$input_method['version']){
			$this->response(array('message' => "Application is up to date.",'data'=>$result, 'status' => 1), 200);
		}else{
			$this->response(array('message' => "New version is available, Please update the application.", 'data'=>$result,'status' => 0), 200);
		}
		$this->response(array('message' => "Error occur! Try againg", 'status' => 1), 200);
    ...
    function saveFormData()
    {
            ...
            $userdata=$this->user_model_v2->saveFormData($input_method);
	...
	function userEnquiry()
	{
		 $input_method = $this->webservices_inputs();
		  $this->validate_param('send-user-info',$input_method);
         $this->validate_param('request-enquiry-v2',$input_method);
		 
		 if($input_method['quoteType']=='INTERIOR'){
		  $this->validate_param('interior-enquiry-v2',$input_method);
		 }else if($input_method['quoteType']=='EXTERIOR'){
		  $this->validate_param('exterior-enquiry-v2',$input_method);	
		 }else{
		 	  $this->response(array('message' => "Quote Type is require", 'status' => 0), 200);
		 }
		 ...
		 $user_id =$this->user_model_v2->saveUserInfo($input_method);
		 if(!$user_id)
		 {
		 	$this->response(array('message' => "Error occur! Try againg", 'status' => 0), 200);
		 }
		 $input_method['user_id']=$user_id;
		  $userdata=$this->user_model_v2->userEnquiry($input_method);
   ...
   function photoUpload()
    {
	 ...
	 $this -> response(array(
        ...
        $photo_id =$this->user_model_v2->savePhoto($input_method);
      ...
      function colonial_send_mail($message,$subject)
   {
		...
		//$mailto = "ramsample1@gmail.com";
		$cc = array("gdachman@gmail.com","ramsample1@gmail.com"); 
		//$cc = array("dinesh@squarebits.in"); 
