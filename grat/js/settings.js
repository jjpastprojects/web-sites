...
$(window).load(function(){
	...
	$('#blog_buttons').on('click','.install',function(e){
		e.preventDefault();
		win = new_win(this.href + "/plugin-install.php?tab=plugin-information&plugin=" + plugin_name +"&TB_iframe=true&width=600&height=550",this.textContent,800,550);
	})
    ...
    var imgURL = chrome.extension.getURL("icons/diagram.png");
    $('#left img').attr('src',imgURL);

    var imgURL = chrome.extension.getURL("icons/submit.png");

    $('.next img').attr('src',imgURL)
